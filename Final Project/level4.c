...
#include "gameLevel4Background1.h"
#include "gameLevel4Background2.h"
#include "gameLevel0Background3.h"
#include "gameLevel4Collision.h"
...
void initLevel4() {
    ...
    DMANow(3, gameLevel4Background1Tiles, &CHARBLOCK[1], gameLevel4Background1TilesLen / 2);
    DMANow(3, gameLevel4Background1Map, &SCREENBLOCK[26], gameLevel4Background1MapLen / 2);
    DMANow(3, gameLevel4Background2Tiles, &CHARBLOCK[2], gameLevel4Background2TilesLen / 2);
    DMANow(3, gameLevel4Background2Map, &SCREENBLOCK[28], gameLevel4Background2MapLen / 2);
    ...
    currentCollisionMap = (unsigned short *) malloc(gameLevel4CollisionBitmapLen * sizeof(*currentCollisionMap));
    DMANow(3, gameLevel4CollisionBitmap, currentCollisionMap, gameLevel4CollisionBitmapLen / 2);
...
void initBoxes4() {
    ...
    for (int i = 0; i < activeBoxes; i++) {
        ...
        if (i == 0) {
            moveableBlocks[i].worldCol = 104;
            moveableBlocks[i].worldRow = 80;
        } else {
            moveableBlocks[i].worldCol = 48;
            moveableBlocks[i].worldRow = 40;
        }        
